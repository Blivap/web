...
function asString(value: unknown, ...alts: unknown[]): string {
...
export function normalizeUser(input: unknown): IUser | null {
  ...
  const hasTokenEnvelope =
    "accessToken" in obj || "access_token" in obj || obj.token !== undefined;
  ...
  const firstname = asString(obj.firstname, obj.firstName, obj.first_name);
